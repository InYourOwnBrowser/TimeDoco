...
- [x] **CI**: `license-checker` is pinned to `25.0.1`, the workflow has a read-only default `permissions:` block with the one job that needs more asking for it, and a `concurrency:` group keyed on the event so a push cannot cancel an in-flight manual deploy. The `on: push` / `workflow_dispatch` asymmetry is deliberate and now carries a comment saying so. *(Still open, deliberately: `npm audit --omit=dev --audit-level=high` can block a deploy on a new upstream advisory — that is the intended gate — and the actions are pinned to mutable major tags rather than commit SHAs.)*
